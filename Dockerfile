...
RUN cd kohya_ss
RUN pip install -r requirements_linux_docker.txt -r requirements.txt

# comment out below if you're not on linux/amd64 - pillow-simd
RUN apt-get update && apt-get install -y --no-install-recommends zlib1g-dev libjpeg62-turbo-dev build-essential && pip uninstall -y pillow && CC="cc -mavx2" pip install -U --force-reinstall pillow-simd
...
# copy pasted from https://github.com/bmaltais/kohya_ss/blob/master/Dockerfile
